...
const getPricesFromCoingecko = async () => {
    const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin%2Csolana%2Cethereum%2Cserum&order=market_cap_desc&per_page=100&page=1&sparkline=false');
    const data = await res.json();
    console.log('data :>> ', data);
    return data
}

const getTotalAssets = async (usdcBalance, btcBalance, ethBalance, wsolBalance) => {
    
    const prices = await getPricesFromCoingecko();
    const btcTotal = (btcBalance*prices.find(t=>t.id==="bitcoin").current_price)
    const ethTotal = (ethBalance*prices.find(t=>t.id==="ethereum").current_price)
    const solTotal = (wsolBalance*prices.find(t=>t.id==="solana").current_price)
...
const SlidingCard = () => {
    ...
    useEffect(() => {
        if (accountMarginData && xenonProgramData && borrowRate) {
            (async () => {
                console.log('accountMarginData :>> ', accountMarginData);
                const assets = (accountMarginData.assets / 10 ** 6).toFixed(2);
                const totalAssets = await getTotalAssets(usdcBalance, btcBalance, ethBalance, wsolBalance);
                ...
                setLiabilites(liabs);
                setUsdcLeftToBorrow(totalAssets - 2 * liabs < 0 ? 0 : totalAssets - 2 * liabs);
                ...
                setHealthPercentage(
                    (
                        (totalAssets / accountMarginData.liabs - 1.2) * 100
                    ).toFixed(0),
                );
                console.log('health :>> ', (totalAssets / accountMarginData.liabs - 1.2) * 100);
            })()
        }
    }, [connected, accountMarginData, xenonProgramData, borrowRate, btcBalance, wsolBalance, ethBalance]);
