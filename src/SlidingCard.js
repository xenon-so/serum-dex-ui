...
const SlidingCard = () => {
    ...
    useEffect(() => {
        if (connected) {
            (async () => {
                console.log('wallet.publicKey 111 :>> ', wallet.walletPublicKey.toBase58());
                const x = await PublicKey.findProgramAddress([wallet.walletPublicKey.toBuffer()], programId);
                setXenonPDA(x[0]);
            })()
        }
    }, [connected])
        ...
        if (connected && xenonPDA != null) {
            ...
            (async () => {
                ...
                try {
                    ...
                    const accounts = await findAssociatedTokenAddress(xenonPDA, new PublicKey(TOKENS.USDC.mintAddress))
                    const walletBalance = await connection.getTokenAccountBalance(accounts, "max")
                    setUsdcBalance(walletBalance.value.uiAmountString);

                    const USDCAccountSubscription = connection.onAccountChange(accounts, (x, y) => {
                        const USDCAccState = ACCOUNT_LAYOUT.decode(x.data);
                        const bal = roundDownTo4Decimals(USDCAccState.amount.toNumber() / 10 ** TOKENS.USDC.decimals)
                        setUsdcBalance(bal);
                ...
                try {
                    const accounts = await findAssociatedTokenAddress(xenonPDA, new PublicKey(TOKENS.BTC.mintAddress))
                    const walletBalance = await connection.getTokenAccountBalance(accounts, "max")
                    setBtcBalance(walletBalance.value.uiAmountString);

                    const USDCAccountSubscription = connection.onAccountChange(accounts, (x, y) => {
                        const USDCAccState = ACCOUNT_LAYOUT.decode(x.data);
                        const bal = roundDownTo4Decimals(USDCAccState.amount.toNumber() / 10 ** TOKENS.BTC.decimals)
                        setBtcBalance(bal);
                        // console.log("state changed ::", USDCAccState, bal)
                    }, 'recent')
                } catch (error) {
                ...
                try {
                    const accounts = await findAssociatedTokenAddress(xenonPDA, new PublicKey(TOKENS.ETH.mintAddress))
                    const walletBalance = await connection.getTokenAccountBalance(accounts, "max")
                    setEthBalance(walletBalance.value.uiAmountString);

                    const USDCAccountSubscription = connection.onAccountChange(accounts, (x, y) => {
                        const USDCAccState = ACCOUNT_LAYOUT.decode(x.data);
                        const bal = roundDownTo4Decimals(USDCAccState.amount.toNumber() / 10 ** TOKENS.ETH.decimals)
                        setEthBalance(bal);
                        // console.log("state changed ::", USDCAccState, bal)
                    }, 'recent')
                } catch (error) {
